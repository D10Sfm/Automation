...
def test_addToCartAccessories():
    driver = DR.initroot()
    driver.get(DO.accessories_url)
    ...
    products_prices = [driver.find_element(By.XPATH,"//tbody/tr[1]/td[4]/span[1]").text,driver.find_element(By.XPATH,"//tbody/tr[2]/td[4]/span[1]").text]
    for i,j in zip(products_nams,DO.prod_names_test_accessories):
        assert i.text == j
    for i,j in zip(products_prices,DO.prod_price_test_accessories):
        assert i == j